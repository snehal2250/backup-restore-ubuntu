...
9. ✅ completed (2026-08-05) — Add VM-based end-to-end restore rehearsal (VirtualBox). **Fully completed**: fresh restore of all 25 declared apps per docs/REHEARSAL-VIRTUALBOX.md §6.7–6.8, cloudflared starts with healthy DNS (enabled + active, 0 failed units), a clean reboot with all services green (`/` and `/etc` verified `root:root 755` after reboot — the perms-fix validation), user dirs restored with the v7 `chats` exclude honored, and an idempotency re-run (everything "already installed (verified via <source>)", exit 0; az's first-run failure was a transient unattended-upgrades dpkg lock, fixed by the re-run, with the azure-cli repo correctly falling back from `resolute` to `noble`). The round caught and fixed **three** real bugs: the schema-v7 `user_dirs` yq lexer regression (`(.path // .)` fix); config restore propagating staging dir metadata onto `/` and `/etc` (new `restore_sync_tree`: `--no-owner --no-group` + dir-mode re-assertion); and **backup.sh pre-creating empty `root/` dirs for every app**, which restore then rsync'd onto `/` — an empty `rsync -a root/ /` still applies the source top-dir's 0770 mode to `/`, killing the restore at app 2/25 (fixed: per-path-only scope dirs in backup.sh, empty-tree skips in `restore_config_tree`/`restore_sync_tree`, and the sudo'd rsync + re-assert running in ONE `bash -c` so the re-assert survives a mode-locked dest root). Two known environmental limitations remain (not repo bugs): VirtualBox NAT DNS flakiness (persistent nmcli upstream fix survives reboot) and snap CLIs under headless guestcontrol (need a GUI session). See docs/PLAN.md Phase 3.
...
- **Recommended next step (updated 2026-08-05):** all P0 items, P1 integrity / conflict-policies / phases / installers / schema / completeness, the P2 test suite, the **P2 catalog/inventory drift refactor (schema v5 `catalog:` + `overrides:`)** and the **VM rehearsal (item 9)** are shipped — the rehearsal completed with cloudflared + clean-reboot verification green, and the working tree carries three rehearsal-finding fixes from the round: the schema-v7 `user_dirs` yq regression, the config-tree dir-metadata propagation onto `/`+`/etc` (`bcc4d3c`, `1679139`), and the empty-`root/`-artifact bug (backup.sh per-path scope dirs + empty-tree skips + single-context sudo re-assert). **P1 config/state/secrets classification is closed as not-required** (curated per-app excludes cover it; the support matrix is now Ubuntu/amd64-only, release unlocked). The only open code item is **P2 avoid backing up auth blindly** (standalone: per-path `include:` + pre-backup secret scanner + `--no-secrets`).
